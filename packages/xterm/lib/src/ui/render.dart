...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  RenderTerminal({
    ...
    String? composingText,
        ...
        _growOnlyResize = growOnlyResize,
  ...
  /// When true, the terminal buffer only grows (never shrinks in height).
  /// This prevents content loss when the viewport temporarily shrinks
  /// (e.g. virtual keyboard appearing on mobile).
  bool _growOnlyResize;
  set growOnlyResize(bool value) {
    if (value == _growOnlyResize) return;
    _growOnlyResize = value;
    markNeedsLayout();
  }
  ...
  void _resizeTerminalIfNeeded() {
    ...
    // Grow-only mode: allow width changes and height increases,
    // but block height decreases. This prevents Buffer.resize() from
    // calling lines.pop() when the viewport shrinks (e.g. keyboard opens),
    // which would cause content loss in TUI apps like Codex CLI.
    if (_growOnlyResize) {
      final currentWidth = _terminal.viewWidth;
      final currentHeight = _terminal.viewHeight;
      final newWidth = _viewportSize!.width;
      final newHeight = _viewportSize!.height;
